...
class PlistReaderRE(DmgMounter):
    ...
    For each key in the plist, there may be a format string to change output format
    ...
    input_variables = {
        ...
        "plist_regex": {
            "required": False,
            "default": { },
            "description": ("Dictionary of plist regex to use. Key names "
                            "should match plist_keys keys. Values "
                            "should be the desired regex for the associated value. "
                            "Defaults to: ",
                            "{ }")
        },
    }
    output_variables = {
        "plist_reader_output_variables": {
            "description": (
                ...
                "processor. Values are changed according to regex.")
        },
    }
        ...
        try:
            ...
            for key, val in keys.items():
                if ( regexs.get(key,'') == '') :
                    ...
                    self.env[val] = re.sub(myreg['pattern'],myreg['repl'],info[key])
                try:
                    self.output(
